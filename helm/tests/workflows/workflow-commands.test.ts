...
import { createEffortState, effortDirective } from "../../src/workflows/effort-command.js";
...
test("/workflows save <name> <runId> saves the specified run", async () => {
  ...
  const { registerWorkflowCommands: reg2 } = await import("../../src/workflows/workflow-commands.js");
  const notified: Array<{ message: string; type?: string }> = [];
  let handler: any;
  reg2(
    {
...
test("/workflows save <name> <runId> warns when run has no script", async () => {
  ...
  let handler: any;
  const { registerWorkflowCommands: reg3 } = await import("../../src/workflows/workflow-commands.js");
  const notified: Array<{ message: string; type?: string }> = [];
  reg3(
    {
      getCommands: () => [{ name: "xxx" }],
