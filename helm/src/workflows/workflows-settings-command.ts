...
export interface WorkflowSettingsCommandOptions {
  /** Project cwd: settings load/save follow the same global+project merge as the editor. */
  cwd?: string;
  /** Standing effort mode shared with `/effort`; edited in place. */
  effort?: EffortState;
  /** Injectable settings store (tests). Defaults to the on-disk store for `cwd`. */
 ...
 */
export function registerWorkflowSettingsCommand(pi: ExtensionAPI, opts: WorkflowSettingsCommandOptions = {}): void {
...
function defaultSettingsStore(cwd?: string): WorkflowSettingsStore {
  if (!cwd) return { load: loadWorkflowSettings, save: saveWorkflowSettings };
  return {
...
async function runSettingsHub(
  ctx: ExtensionCommandContext,
  opts: WorkflowSettingsCommandOptions,
  ...
  while (true) {
    ...
    if (opts.effort) entries.push(`Effort — ${opts.effort.level}`);
    entries.push("Close");
...
async function editKeywordTrigger(ctx: ExtensionCommandContext, store: WorkflowSettingsStore): Promise<void> {
  ...
  if (choice === "Turn on" || choice === "Turn off") {
    ...
    ctx.ui.notify(
      saved
        ...
        : `Could not save the keyword trigger preference.`,
      "info",
    );
    return;
  }
...
async function editProgressMode(ctx: ExtensionCommandContext, store: WorkflowSettingsStore): Promise<void> {
  const choice = await ctx.ui.select(`Progress panel (currently ${loadProgressMode(store)})`, ["compact", "detailed"]);
  if (choice !== "compact" && choice !== "detailed") return;
  ...
  ctx.ui.notify(
    saved
      ...
      : "Could not save the progress panel preference.",
    "info",
  );
}

async function editEffort(ctx: ExtensionCommandContext, effort?: EffortState): Promise<void> {
  if (!effort) return;
  const choice = await ctx.ui.select(`Effort (currently ${effort.level})`, ["off", "high", "ultra"]);
  if (choice !== "off" && choice !== "high" && choice !== "ultra") return;
  effort.level = choice;
  ctx.ui.notify(
    choice === "off"
      ? "Effort off — messages are no longer auto-armed as workflows."
      : `Effort ${choice} — substantive messages now auto-arm a workflow. Use /effort off to stop.`,
    "info",
  );
}
...
function tiersLabel(ctx: ExtensionCommandContext): string {
...
function triggerLabel(store: WorkflowSettingsStore): string {
...
function loadTrigger(store: WorkflowSettingsStore): { enabled: boolean; word: string } {
  try {
...
function loadProgressMode(store: WorkflowSettingsStore): "compact" | "detailed" {
  try {
...
function loadProgressMaxAgents(store: WorkflowSettingsStore): number {
  try {
...
function persist(store: WorkflowSettingsStore, settings: WorkflowSettings): boolean {
  try {
    store.save(settings);
    return true;
  } catch {
