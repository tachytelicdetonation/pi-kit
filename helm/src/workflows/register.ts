...
export function registerWorkflows(pi: ExtensionAPI, opts: { presence: HelmPresence }): { manager: WorkflowManager } {
  ...
  const workflowTool = createWorkflowTool({
    ...
    async reviewLaunch(review) {
      ...
      if (ds) {
        ...
        if (idx === 0) return { approved: true, script: review.script };
        if (idx === 1) { approvalStore.approve(identity); return { approved: true, script: review.script }; }
