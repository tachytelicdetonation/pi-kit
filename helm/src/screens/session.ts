...
export function renderSession(
  ...
  session.lines.forEach((line, index) => {
    ...
    if (line.expandable && expanded.has(index)) lines.push(...peekRows(theme, line, w));
  });
...
function verbColor(verb: ActivityLine["verb"]): PaletteColor {
  switch (verb) {
    case "context":
...
function activityRow(theme: ThemeLike, line: ActivityLine, isSelected: boolean, isExpanded: boolean, width: number): string {
...
function resultText(theme: ThemeLike, line: ActivityLine): string {
  if (line.verb === "edit") {
  ...
  if (line.verb === "verify") return paint(theme, PALETTE.success, line.result);
 ...
 */
function paintPeek(theme: ThemeLike, peek: string): string {
  const match = peek.match(/^(\s*\d+)\s+([+−-])\s(.*)$/);
  if (!match) return paint(theme, PALETTE.dim, peek);
