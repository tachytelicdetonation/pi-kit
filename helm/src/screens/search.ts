...
import type { SearchKind, SearchResult } from "./../data/source.js";
...
function kindColor(kind: SearchKind): PaletteColor {
  switch (kind) {
    case "goal":
...
export function renderSearch(
  ...
  if (count) lines.push(truncateToWidth(`  ${paint(theme, PALETTE.dim, count)}`, w, ""));
  lines.push("");

  // ── The result list ──────────────────────────────────────────────────────
  let anchor = 0;
  if (results.length === 0) {
    if (query) lines.push(truncateToWidth(`  ${paint(theme, PALETTE.dim, "no matches")}`, w, ""));
  } else {
...
function resultRow(theme: ThemeLike, result: SearchResult, isSelected: boolean, width: number): string {
