...
export function renderDocument(lines: string[], theme: ThemeLike, width: number, height: number): string[] {
  const w = Number.isFinite(width) ? Math.max(0, Math.floor(width)) : 0;
  const h = Number.isFinite(height) ? Math.max(0, Math.floor(height)) : 0;
  if (!w || !h) return [];
  const body = lines.flatMap((line) => wrapPlain(line, Math.max(1, w - 4)).map((part) =>
    truncateToWidth(`  ${paint(theme, PALETTE.mid, part)}`, w, "…"),
  ));
