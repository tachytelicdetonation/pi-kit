...
export function renderMissionControl(
  ...
  if (needsYouCount(state) > 0) {
    lines.push(
      headerLine(
        theme,
        `${paint(theme, PALETTE.warning, `${GLYPH.needsYou} needs you`)} ${paint(theme, PALETTE.dim, `(${state.escalations.length})`)}`,
        w,
      ),
    );
  ...
  if (state.goals.length > 0) lines.push("");
  ...
  if (state.loops.length > 0) {
    lines.push(
      headerLine(
        theme,
        `${paint(theme, PALETTE.success, `${GLYPH.loops} loops`)} ${paint(theme, PALETTE.dim, `(${activeLoopCount(state)} active)`)}`,
        w,
      ),
    );
...
function escalationRow(theme: ThemeLike, escalation: Escalation, num: number, isSelected: boolean, width: number): string {
...
function workflowState(state: Workflow["state"]): { glyph: string; text: string; color: PaletteColor } {
  switch (state) {
    case "running":
...
function loopStatus(theme: ThemeLike, loop: Loop): string {
  ...
  if (loop.lastFired) return paint(theme, PALETTE.dim, `idle · last fired ${loop.lastFired}`);
  if (loop.nextRun) return paint(theme, PALETTE.dim, `next run ${loop.nextRun}`);
  return paint(theme, PALETTE.dim, "idle");
}

function loopRow(theme: ThemeLike, loop: Loop, isSelected: boolean, width: number): string {
