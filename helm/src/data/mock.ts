 ...
 */
export function seedCodemodDrillIn(): WorkflowDetail {
  return {
...
export function genericDrillIn(workflowId: string, goalId: string, name: string): WorkflowDetail {
  return {
    workflowId,
 ...
 */
export function seedSession(worktreeId: string): Session {
  return {
    worktreeId,
 ...
 */
export function seedExportMapEscalation(): Escalation {
  return {
    id: "e-export-map",
    ...
    idleCost: "$0.40 idle",
    problem:
      "pkg/api's ESM rewrite requires touching exports.map.json — you told me to avoid it: platform team merged a conflicting change 40 min ago.",
    evidence: [
...
export function seedApiRenameEscalation(): Escalation {
  return {
    id: "e-api-rename",
 ...
 */
export function seedDigest(): DigestData {
  return {
 ...
 */
export function seedIntake(draftId = "d-esm"): IntakeDraft {
  return {
    id: draftId,
 ...
 */
export function seedLoopDraft(id = "l-draft-gh-issues", name = "gh-issues"): LoopDraft {
  return {
    id,
    name,
...
const CLOSEOUT_NAMES: Record<string, string> = {
 ...
 * re-proposed.
 */
export function seedCloseout(goalId = "g-esm"): Closeout {
  return {
    goalId,
...
export function seedUsageDetail(): UsageDetail {
  return {
    providers: [
 ...
 */
export function seedSearchCorpus(): SearchResult[] {
  return [
...
export function seedState(): HelmState {
  return {
...
export class MockDataSource implements DataSource {
   ...
   */
  constructor(options: { tick?: boolean; shouldShowDigest?: boolean } = {}) {
  ...
  getDrillIn(workflowId: string): WorkflowDetail | undefined {
    if (workflowId === "w-codemod") return seedCodemodDrillIn();
  ...
  search(query: string): SearchResult[] {
    ...
    const live: SearchResult[] = this.store.getState().precedents.map((precedent) => ({
  ...
  async execute(command: HelmCommand): Promise<HelmCommandResult> {
    switch (command.type) {
      ...
      case "goal.answer": {
        ...
        if (draft) this.intakes.set(command.draftId, { ...draft, userReply: command.text, openQuestions: false });
      ...
      case "workflow.togglePause": {
        ...
        if (lane?.state === "paused") this.store.resumeWorkflow(lane.id);
        else this.store.pauseWorkflow(command.workflowId);
        return { ok: true };
      }
      ...
      case "loop.edit": {
        ...
        if (draft) this.loopDrafts.set(command.loopId, { ...draft, prompt: command.text, steps: command.text, trialPassed: false });
        return { ok: Boolean(draft) };
      }
