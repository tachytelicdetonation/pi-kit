 ...
 */
export function header(theme: ThemeLike, context: string, right: string, width: number): string {
...
export function divider(theme: ThemeLike, width: number): string {
 ...
 */
export function spring(theme: ThemeLike, left: string, right: string, width: number): string {
  const w = safeWidth(width);
  if (w <= 0) return "";
  ...
  if (rightWidth >= w) return truncateToWidth(right, w, "…");
 ...
 */
export function column(text: string, width: number, align: "left" | "right" = "left"): string {
  const w = safeWidth(width);
  if (w <= 0) return "";
  const current = visibleWidth(text);
  if (current > w) return truncateToWidth(text, w, "…");
 ...
 */
export function windowLines(theme: ThemeLike, lines: string[], anchor: number, height: number, width: number): string[] {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (nextTop === topHint && nextBottom === bottomHint) break;
    topHint = nextTop;
  ...
  const hint = (arrow: string, count: number): string =>
    truncateToWidth(`  ${paint(theme, PALETTE.dim, `${arrow} ${count}`)}`, width, "");

  const out: string[] = [];
  if (topHint) out.push(hint("↑", hiddenAbove));
  for (let i = start; i < start + band; i++) out.push(lines[i]);
  if (bottomHint) out.push(hint("↓", hiddenBelow));
