...
import type { ProviderId, ProviderViewState, UsageViewModel } from "./types.js";
 ...
 * needed: all footer color is emitted as raw truecolor ANSI (with a 256-color
 * fallback), following the palette below rather than the theme's named roles.
 */
interface ThemeLike {
  getColorMode?(): "truecolor" | "256color";
}

/** A footer color: exact truecolor plus a reasonable xterm-256 fallback. */
...
/**
 * Footer render model: provider snapshots plus the ambient session facts the
 * host exposes (cwd, branch, context %, cost, model, effort).
 */
...
export class UsageFooterComponent implements Component {
  constructor(
    private readonly getModel: () => FooterViewModel,
...
export function renderFooter(model: FooterViewModel, theme: ThemeLike, width: number): string[] {
  ...
  if (safeWidth < 8) return [truncateToWidth("usage", safeWidth, "")];
  ...
  if (visibleWidth(line) > safeWidth) line = truncateToWidth(line, safeWidth, "");
  return [line];
}
...
function buildBarGroup(theme: ThemeLike, states: ProviderViewState[], cells: number): string {
  const { colored, empty, totalPercent, known } = computeStackedBar(states, cells);
  const chunks = PROVIDER_ORDER.map((provider, index) =>
    paint(theme, PROVIDER_COLORS[provider], BAR_GLYPH.repeat(colored[index] ?? 0)),
  );
...
function buildRightGroup(theme: ThemeLike, model: FooterViewModel, showCost: boolean, showEffort: boolean): string {
  const head: string[] = [];
  if (showCost && model.costUsd !== undefined) head.push(paint(theme, PALETTE.dim, formatCost(model.costUsd)));
...
function paint(theme: ThemeLike, color: PaletteColor, text: string): string {
  if (!text) return "";
  if (theme.getColorMode?.() === "256color") return `[38;5;${color.xterm}m${text}[39m`;
  const [red, green, blue] = color.rgb;
  return `[38;2;${red};${green};${blue}m${text}[39m`;
}

export function formatUsageDetails(view: UsageViewModel): string {
  ...
  for (const provider of PROVIDER_ORDER) {
    ...
    if (state?.error) lines.push(`  Last refresh: ${state.error}`);
  }
  return lines.join("\n");
}
