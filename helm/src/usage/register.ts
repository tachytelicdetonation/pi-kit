...
import type { FooterController } from "../host/footer-controller.js";
import { resolveUsagePaths } from "./paths.js";
import { UsageService } from "./service.js";
import { formatUsageDetails, UsageFooterComponent, type FooterViewModel } from "./ui.js";

const REFRESH_MINUTES = Math.min(
  60,
  Math.max(5, Number(process.env.PI_USAGE_HEALTH_REFRESH_MINUTES) || 30),
);

export interface UsageHealthHandle {
  service: UsageService;
  /** Fan-out of the service's onUpdate; returns unsubscribe. */
  subscribe(cb: () => void): () => void;
}

export function registerUsageHealth(pi: ExtensionAPI, opts: { footer: FooterController }): UsageHealthHandle {
  let requestRender = () => {};
  ...
  const service = new UsageService({
    ...
    onUpdate: () => {
      if (tuiActive) requestRender();
      for (const l of listeners) l();
    },
  });

  const buildFooterModel = (footerData?: ReadonlyFooterDataProvider): FooterViewModel => {
  ...
  const installFooter = (ctx: ExtensionContext) => {
    const factory: Parameters<ExtensionContext["ui"]["setFooter"]>[0] = (tui, theme, footerData) => {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    void (async () => {
      ...
      if (!tuiActive || generation !== activeGeneration) return;
  ...
  pi.on("agent_settled", async (_event, ctx) => {
    if (ctx.mode !== "tui" || !tuiActive) return;
    currentCtx = ctx;
 ...
 */
function computeSessionCost(ctx: ExtensionContext | undefined): number | undefined {
