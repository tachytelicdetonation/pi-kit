...
export async function fetchKimiUsage(options: KimiOptions): Promise<UsageSnapshot> {
  ...
  if (!credential) throw new Error("Kimi Code is not authenticated");
  ...
  if (!response.ok) throw new Error(`Kimi usage returned HTTP ${response.status}`);
  return parseKimiUsagePayload(await response.json(), now());
}

export function parseKimiUsagePayload(payload: unknown, fetchedAt = Date.now()): UsageSnapshot {
  ...
  if (buckets.length === 0) throw new Error("Kimi usage response contained no quota windows");
  return { provider: "kimi", source: "live", fetchedAt, buckets };
}

interface KimiTokenWire {
  access_token: string;
...
export async function ensureFreshKimiCredential(
  paths: UsagePaths,
  now: () => number,
  fetchImpl: FetchLike,
  timeoutMs = 8_000,
  ...
  if (initial.expires_at * 1_000 > now() + 5 * 60_000) return initial.access_token;
  ...
  try {
    ...
    if (current.expires_at * 1_000 > now() + 5 * 60_000) return current.access_token;
    if (!current.refresh_token) throw new Error("Kimi OAuth credential has no refresh token; run /login in Kimi Code");
    ...
    if (!response.ok) throw new Error(`Kimi OAuth refresh returned HTTP ${response.status}; run /login in Kimi Code`);
...
async function readKimiToken(path: string): Promise<KimiTokenWire | undefined> {
  try {
...
async function writeKimiToken(path: string, token: KimiTokenWire): Promise<void> {
...
function addUsageRow(
  ...
  if (!record || limit === undefined || limit <= 0 || used === undefined) return;
...
function windowLabel(window: Record<string, unknown> | undefined): string | undefined {
  const duration = finiteNumber(window?.duration);
  const unit = stringValue(window?.timeUnit)?.toUpperCase();
  if (duration === undefined) return undefined;
  if (unit?.includes("MINUTE")) return duration >= 60 && duration % 60 === 0 ? `${duration / 60}-hour` : `${duration}-minute`;
  if (unit?.includes("HOUR")) return `${duration}-hour`;
  if (unit?.includes("DAY")) return `${duration}-day`;
  return `${duration}-second`;
}

function toMinutes(duration: number, unit: string | undefined): number | undefined {
