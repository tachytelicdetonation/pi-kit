...
export type SnapshotSource = "live" | "provider-cache" | "extension-cache";
...
export const PROVIDER_ORDER: ProviderId[] = ["codex", "claude", "kimi"];

/** Relative plan-capacity units chosen by the user: $200 plans vs tinkering plans. */
export const PROVIDER_UNITS: Record<ProviderId, number> = {
  codex: 20,
  claude: 20,
  kimi: 1,
};
...
export function providerRemaining(snapshot: UsageSnapshot | undefined): number | undefined {
  if (!snapshot) return undefined;
  const buckets = snapshot.buckets.filter(
    (bucket) => bucket.affectsHealth !== false && Number.isFinite(bucket.usedPercent),
  );
  if (buckets.length === 0) return undefined;
  return Math.min(...buckets.map((bucket) => 100 - clampPercent(bucket.usedPercent)));
}

export function aggregateRemaining(states: ProviderViewState[]): number | undefined {
  ...
  if (knownProviders === 0) return undefined;
