...
const PLAN_COL = 16;
export function renderIntake(draft, theme, width, height) {
    ...
    const planHead = `${paint(theme, PALETTE.label, "plan")} ${paint(theme, PALETTE.dim, `· ${draft.planWorkflows.length} workflows`)}`;
...
function promptEcho(theme, text, width) {
...
function planRow(theme, name, description, modelTag, width) {
 ...
 */
function actionLine(theme, draft, width) {
    const dot = ` ${paint(theme, PALETTE.dim, "·")} `;
    const left = `${paint(theme, PALETTE.dim, "est")} ${paint(theme, PALETTE.label, draft.estWall)}` +
    ...
    const right = `${goKey}${dot}${paint(theme, PALETTE.brand, "e")} ${paint(theme, PALETTE.dim, "edit plan")}` +
