...
export function renderEscalation(escalation, theme, width, height) {
    ...
    const hint = `${paint(theme, PALETTE.brand, range)} ${paint(theme, PALETTE.dim, "decide")}` +
    ...
    if (escalation.precedentPhrase) {
        const remind = `${paint(theme, PALETTE.dim, "your call becomes precedent: ")}` +
 ...
 */
function paintEvidenceLine(theme, raw) {
    const diff = raw.match(/^([+-])\s(.*)$/);
    if (!diff)
        return paint(theme, PALETTE.dim, raw);
