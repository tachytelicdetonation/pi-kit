...
export function renderDrillInCounts(theme, detail) {
...
function testStateStatus(state) {
    switch (state) {
        case "green":
...
export function renderDrillIn(detail, theme, width, height, selection) {
    ...
    lines.push(headerLine(`${paint(theme, PALETTE.label, "worktrees")} ${paint(theme, PALETTE.mid, "fix → review → apply")} ${paint(theme, PALETTE.dim, "(each chip = one agent)")}`, w));
    ...
    if (withTicks.length > 0) {
        lines.push(headerLine(`${paint(theme, PALETTE.label, "race to green, by package")} ${paint(theme, PALETTE.dim, "(each tick = a test run)")}`, w));
        for (const worktree of withTicks)
            lines.push(raceRow(theme, worktree, w));
        lines.push("");
    }
    // ── Footer note ─────────────────────────────────────────────────────────
    lines.push(headerLine(paint(theme, PALETTE.dim, "no read/edit/bash here either — enter on a worktree opens its session view (4a)"), w));
    return windowLines(theme, lines, anchor, h, w);
}
/** The big remaining count + `<unit> left · from N · burn N/hr · done ETA`. */
function burnDownRow(theme, detail, width) {
    const count = paint(theme, PALETTE.bright, formatCount(detail.queueRemaining));
    const tail = `${paint(theme, PALETTE.label, `${detail.queueUnit} left`)}` +
...
function worktreeRow(theme, worktree, isSelected, width) {
...
function renderChips(theme, worktree) {
    const stages = ["fix", "review", "apply"];
...
function raceRow(theme, worktree, width) {
    const pkg = paint(theme, PALETTE.mid, column(worktree.package, PKG_COL));
...
function headerLine(content, width) {
