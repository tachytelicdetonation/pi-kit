...
const LABEL_COL = 13;
export function renderCloseout(closeout, theme, width, height, view = {}) {
    ...
    lines.push(clip(`${paint(theme, PALETTE.label, "precedents")} ${paint(theme, PALETTE.dim, "· proposed for CLAUDE.md / skills")}`, w));
    ...
    if (view.applied) {
        lines.push(clip(`${paint(theme, PALETTE.success, "✓")} ${paint(theme, PALETTE.dim, "precedents applied to CLAUDE.md / skills")}`, w));
    }
    else {
 ...
 */
function receiptLine(theme, c, width) {
    const count = paint(theme, PALETTE.bright, `${c.packagesDone}/${c.packagesTotal} ${c.unit}`);
    const dot = ` ${paint(theme, PALETTE.dim, "·")} `;
    const stats = `${paint(theme, PALETTE.success, c.addedText)} ${paint(theme, PALETTE.error, c.removedText)}` +
...
function actionLine(theme, width) {
    const dot = ` ${paint(theme, PALETTE.dim, "·")} `;
