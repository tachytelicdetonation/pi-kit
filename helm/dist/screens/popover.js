...
function bar(theme, color, remaining) {
...
function contentRows(detail, theme) {
    const rows = [];
    for (const provider of detail.providers) {
        ...
        const painted = `${paint(theme, providerColor(provider.id), name)} ${bar(theme, providerColor(provider.id), provider.remaining)} ` +
    ...
    rows.push({
        plain: spend,
        painted: `${paint(theme, PALETTE.dim, "spend")}  ${paint(theme, PALETTE.dim, "today")} ${paint(theme, PALETTE.label, detail.spendToday)} ` +
 ...
 * top border: `┌ usage ─…─┐`.
 */
export function usagePopoverLines(detail, theme, width) {
    ...
    const border = (s) => paint(theme, PALETTE.inputBorder, s);
    const lines = [];
 ...
 */
export function overlayPopover(body, box, width) {
    const w = Number.isFinite(width) ? Math.max(0, Math.floor(width)) : 0;
    if (box.length === 0 || body.length === 0 || w <= 0)
        return body;
    ...
    const out = [...body];
    for (let i = 0; i < visibleBox.length; i++)
        out[start + i] = visibleBox[i];
    return out;
}
