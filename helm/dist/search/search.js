...
export function resolveEffort(input) {
    ...
    let score = 0;
    if (input.query.length > 140 || countWords(input.query) > 24)
        score += 1;
    if (input.query.length > 360 || countWords(input.query) > 60)
        score += 1;
    if (/\b(compare|contrast|evaluate|investigate|research|cross[- ]?check|timeline|tradeoffs?|why)\b/.test(query))
        score += 2;
    if (/\b(latest|current|today|recent|breaking|as of|this (week|month|year))\b/.test(query))
        score += 1;
    if (/\b(medical|legal|financial|security|vulnerability|safety|scientific)\b/.test(query))
        score += 2;
    if ((input.maxResults ?? 5) > 5)
        score += 1;
    if (input.dateFrom || input.dateTo || input.freshness)
        score += 1;
    if ((input.includeDomains?.length ?? 0) + (input.excludeDomains?.length ?? 0) > 2)
        score += 1;
    if ((input.exactTerms?.length ?? 0) + (input.excludeTerms?.length ?? 0) > 2)
        score += 1;
    if (score >= 5)
        return "high";
    if (score >= 2)
...
export function resolveDepth(input, effort) {
...
function normalizedDomains(values, label) {
    ...
    return [...new Set(normalized.filter((value) => Boolean(value)))].slice(0, 20);
}
...
export function buildCodexArgs(input, paths, options) {
    ...
    if (location)
        args.push(...configArg("tools.web_search.location", location));
    args.push("--output-schema", paths.schema, "-o", paths.output, "-");
    return args;
}
function safeStrings(value, limit, maxChars) {
...
export function validateAndFilterPayload(value, input) {
    if (!value || typeof value !== "object")
        throw new Error("Codex returned a non-object search result");
    const record = value;
    if (typeof record.answer !== "string" || !record.answer.trim())
        throw new Error("Codex returned no search answer");
    const includeDomains = normalizedDomains(input.includeDomains, "includeDomains");
    ...
    for (const item of Array.isArray(record.sources) ? record.sources : []) {
        ...
        seen.add(canonical);
        sources.push(source);
        if (sources.length >= Math.min(Math.max(input.maxResults ?? 5, 1), 10))
            break;
    }
    const limitations = safeStrings(record.limitations, 12, 600);
    if (filteredSources > 0)
        limitations.push(`${filteredSources} source(s) were removed by URL validation or domain filters.`);
    return {
...
export function buildSearchPrompt(input, effort, depth) {
    const request = {
        query: input.query,
        effort,
        searchDepth: depth,
...
async function runProcess(executable, args, input, options) {
    return new Promise((resolve, reject) => {
        ...
        child.stdout.on("data", (chunk) => {
            ...
            if (Buffer.byteLength(stdout) > MAX_CAPTURE_BYTES)
                terminate(new Error("Codex stdout exceeded the 2MB safety limit"));
        });
        child.stderr.on("data", (chunk) => {
            ...
            if (Buffer.byteLength(stderr) > MAX_CAPTURE_BYTES)
                terminate(new Error("Codex stderr exceeded the 2MB safety limit"));
        });
...
export async function runCodexSearch(input, options = {}) {
    ...
    try {
        ...
        if (Buffer.byteLength(statelessResult) > MAX_RESULT_BYTES)
            throw new Error("Codex result exceeded the 256KB safety limit");
