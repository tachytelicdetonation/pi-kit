...
const searchParameters = Type.Object({
    ...
    safeSearch: Type.Optional(StringEnum(["strict", "moderate", "off"], {
        description: "Requested content filtering level. This is best effort because Codex exposes no hard SafeSearch switch.",
    })),
    timeoutSeconds: Type.Optional(Type.Integer({ minimum: 15, maximum: 600, description: "Hard subprocess timeout. Defaults vary by effort." })),
});
...
export function formatSearchResult(result) {
    ...
    sections.push(`_Codex ${result.meta.model} · ${result.meta.effort} effort · ${result.meta.depth} search depth · ${(result.meta.elapsedMs / 1_000).toFixed(1)}s · ${formatNumber(result.meta.tokensUsed)} tokens_`);
...
async function searchWithLimit(input, signal, onProgress) {
...
export function registerCodexSearch(pi) {
    pi.registerTool({
        name: "codex_search",
        label: "Codex Search",
        description: "Perform complex, current, or multi-source web research in an isolated ephemeral Codex CLI session using GPT-5.6 Sol. Supports Google-like filters; date, language, file type, term, and SafeSearch controls are best effort. Returned URLs are validated and output is limited to 45KB.",
        ...
        async execute(_toolCallId, params, signal, onUpdate) {
            const result = await searchWithLimit(params, signal, (message) => {
