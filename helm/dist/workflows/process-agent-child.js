...
import { createAgentSession, DefaultResourceLoader, getAgentDir, ModelRegistry, ModelRuntime, SessionManager, SettingsManager, } from "@earendil-works/pi-coding-agent";
...
process.on("message", (message) => {
    if (message.type === "abort" && message.nonce === nonce)
    ...
    if (message.type === "tool-response") {
        ...
        if (message.ok)
            ...
            request.reject(new Error(message.error?.message ?? "Parent tool execution failed"));
        return;
    }
    if (message.type === "init" && !nonce)
        void run(message);
});
        ...
        tools.push({
            ...
            async execute(_id, input) {
                ...
                if (!Check(schema, converted))
                    throw new Error("structured_output did not match the required schema");
    ...
    try {
        ...
        if (init.schema && !structuredCalled) {
            ...
            for (let attempt = 0; attempt < retries && !structuredCalled; attempt++) {
                await session.prompt("You did not call structured_output. Call structured_output now as your only action with the required fields.");
                throwIfProviderLimit(session.messages, init.label);
            }
        }
        ...
        if (result === undefined || (typeof result === "string" && !result.trim())) {
            throw new Error(init.schema ? "Subagent did not produce valid structured_output" : "Subagent produced no assistant output");
        }
...
function proxyTool(init, descriptor) {
    return {
...
function extractStructured(messages, rawSchema) {
