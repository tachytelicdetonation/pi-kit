...
process.on("message", (message) => {
    if (message.type === "response") {
        if (!activeNonce || message.nonce !== activeNonce)
            return;
        ...
        if (typeof message.budgetSpent === "number")
            spent = Math.max(spent, message.budgetSpent);
        if (message.ok)
            request.resolve(message.value);
        else
            request.reject(remoteError(message.error));
        return;
    }
    if (message.type === "init" && !activeNonce)
...
async function execute(init) {
    ...
    const agent = (prompt, options = {}) => request("agent", [
        prompt,
    ...
    const phase = (title, options) => {
        currentPhase = String(title);
        notify("phase", [currentPhase, options]);
    };
    const log = (message) => notify("log", [String(message)]);
    const workflow = (nameOrScript, childArgs) => request("workflow", [nameOrScript, childArgs]);
    const checkpoint = (promptText, options) => request("checkpoint", [promptText, options]);
    ...
    const verify = async (item, options = {}) => {
        ...
        const votes = (await parallel(Array.from({ length: reviewers }, (_, index) => () => agent(`Adversarially review whether the following is REAL/correct. Try to refute it; default to real=false if unsure.${lenses.length ? ` Focus lens: ${lenses[index % lenses.length]}.` : ""}\n\n${claim}`, { label: `verify ${index + 1}`, schema })))).filter(Boolean);
        ...
        if (real)
            quality.verify.confirmed++;
        return { real, realCount, total: votes.length, votes };
    };
    const judgePanel = async (attempts, options = {}) => {
        ...
        const scored = (await parallel(attempts.map((attempt, attemptIndex) => async () => {
            const text = typeof attempt === "string" ? attempt : JSON.stringify(attempt);
            const judgments = (await parallel(Array.from({ length: judges }, (_, judgeIndex) => () => agent(`Score this candidate from 0 to 1 on: ${rubric}. Reply with the score.\n\nCandidate:\n${text}`, {
        ...
        if (best && typeof best.score === "number")
            quality.judge.bestScore = Math.max(quality.judge.bestScore, best.score);
        return best;
    };
    const loopUntilDry = async (options) => {
        if (!options || typeof options.round !== "function")
            throw new TypeError("loopUntilDry requires { round }");
        const key = options.key ?? ((value) => JSON.stringify(value));
        const seen = new Set();
        const all = [];
        ...
        for (let round = 0; round < (options.maxRounds ?? 50) && dry < requiredDry; round++) {
            ...
            try {
                ...
                if (code === "TOKEN_BUDGET_EXHAUSTED" || code === "AGENT_LIMIT_EXCEEDED")
                    break;
                throw error;
            }
    ...
    const retry = async (thunk, options = {}) => {
        ...
        for (let attempt = 0; attempt < Math.max(1, options.attempts ?? 3); attempt++) {
            ...
            if (!options.until || options.until(last))
                return last;
        }
        return last;
    };
    const gate = async (thunk, validator, options = {}) => {
        ...
        for (let attempt = 0; attempt < attempts; attempt++) {
            ...
            if (verdict?.ok)
                return { ok: true, value: last, attempts: attempt + 1 };
            feedback = verdict?.feedback;
        }
        return { ok: false, value: last, attempts };
    };
    const consoleBridge = Object.freeze({
        log: (...values) => log(values.map(String).join(" ")),
        info: (...values) => log(values.map(String).join(" ")),
        warn: (...values) => log(`[warn] ${values.map(String).join(" ")}`),
        error: (...values) => log(`[error] ${values.map(String).join(" ")}`),
    });
    ...
    try {
        ...
        for (const request of pending.values())
            request.reject(new Error("Workflow sandbox stopped"));
        pending.clear();
    }
}
function isNonRecoverable(error) {
    return Boolean(error && typeof error === "object" && error.recoverable === false);
}
