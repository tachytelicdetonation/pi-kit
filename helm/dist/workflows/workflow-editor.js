 ...
 */
export function colorizeWorkflow(line, tick, palette = RAINBOW, triggerWord = DEFAULT_KEYWORD_TRIGGER_WORD) {
    const tokens = tokenizeAnsi(line);
    const visible = tokens
        .filter((t) => t.ch !== undefined)
        .map((t) => t.ch)
        .join("");
    if (!hasTrigger(visible, triggerWord))
        return line;
    const ranges = [];
 ...
 */
export function customEditorConstructorArgs(tui, theme, keybindings, baseEditorCtor = Object.getPrototypeOf(CustomEditor)) {
    ...
    if (baseEditorCtor.length !== 2 && !warnedUnexpectedArity) {
        ...
        console.warn(`[pi-dynamic-workflows] WorkflowEditor: base editor constructor takes ${baseEditorCtor.length} required ` +
...
export class WorkflowEditor extends CustomEditor {
    ...
    disabled = false;
    wasTriggered = false;
    constructor(hostTui, theme, keybindings, modeState) {
    ...
    isActive() {
        return (this.modeState.keywordTriggerEnabled &&
            !this.disabled &&
    ...
    handleInput(data) {
        ...
        if (after !== before) {
            ...
            const suppressionCleared = this.modeState.suppressedKeywordText !== undefined &&
                normalizedAfter !== "" &&
            ...
            if (now && (!this.wasTriggered || suppressionCleared))
                this.disabled = false;
    ...
    render(width) {
        ...
        try {
            ...
            if (!WorkflowEditor.warnedRenderFallback) {
                ...
                console.warn("[pi-dynamic-workflows] WorkflowEditor: base editor render() threw; degrading to a minimal, unstyled " +
    ...
    static warnedRenderFallback = false;
    /**
...
export function registerWorkflowProgressCommands(pi, settingsStore = DEFAULT_SETTINGS_STORE) {
    pi.registerCommand?.("workflows-progress", {
        ...
        async handler(args, _ctx) {
            ...
            await say(`Workflow progress panel is ${loadProgressMode(settingsStore)}. Usage: /workflows-progress compact | detailed | status`);
        },
    });
...
export function installWorkflowEditor(pi, ui, effort, options = {}) {
