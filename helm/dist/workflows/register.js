...
import { createEffortState, createHostWorkflowContext, createParentRoutedPermissionBroker, createWorkflowApprovalStore, createWorkflowStorage, createWorkflowTool, installResultDelivery, installTaskPanel, installWorkflowEditor, loadWorkflowSettings, registerAllSavedWorkflows, registerBuiltinWorkflows, registerEffortCommand, registerWorkflowCommands, registerWorkflowModelsCommand, registerWorkflowSettingsCommand, saveWorkflowSettingsForCwd, UsageLimitScheduler, WorkflowManager, workflowLaunchApprovalRequirement, } from "./index.js";
export function registerWorkflows(pi, opts) {
    ...
    let lastInputOrigin = "interactive";
    pi.on("input", (event) => {
    ...
    const workflowTool = createWorkflowTool({
        ...
        async reviewLaunch(review) {
            ...
            if (ds) {
                const idx = await bridgePermissionRequest(ds, {
                    source: { kind: "workflow", label: `workflow › ${review.workflowName}` },
                    question: `Run generated workflow "${review.workflowName}"?`,
                    options: ["Run once", "Always allow this workflow", "Deny"],
                });
                if (idx === 0)
                    return { approved: true, script: review.script };
                if (idx === 1) {
        ...
        createHostContext(ctx, signal) {
            ...
            // Inner tool-permission brokering keeps host UI until helm models tool-grain prompts.
    ...
    registerAllSavedWorkflows(pi, cwd, storage, manager, { commandPrefix: "wf-" });
