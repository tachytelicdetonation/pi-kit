...
export function registerSavedWorkflow(pi, cwd, wf, manager, exists, options) {
    const commandName = `${options?.commandPrefix ?? ""}${wf.name}`;
    if (isRegistered(pi, commandName))
        return;
...
export function registerAllSavedWorkflows(pi, cwd, storage, manager, options) {
    for (const wf of storage.list()) {
        registerSavedWorkflow(pi, cwd, wf, manager, () => storage.list().some((w) => w.name === wf.name), options);
    }
}
