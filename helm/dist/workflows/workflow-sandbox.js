...
export function runWorkflowSandbox(options) {
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on("exit", (code, signal) => {
            if (!settled) {
                finish(new Error(`Workflow sandbox exited before returning a result (code=${String(code)}, signal=${String(signal)})${stderr ? `: ${stderr.trim()}` : ""}`));
            }
        });
        child.on("message", (message) => {
            ...
            if (message.type === "result") {
                if (message.ok) {
                    if (message.quality !== undefined)
                        options.onQuality?.(message.quality);
            ...
            if (message.type === "request")
                void handleRequest(child, message, options.handlers, nonce);
        });
...
async function handleRequest(child, request, handlers, nonce) {
    try {
        let value;
        switch (request.method) {
            ...
            case "workflow":
                ...
                break;
            case "checkpoint":
                value = await handlers.checkpoint(assertString(request.args[0], "checkpoint prompt"), asRecord(request.args[1]));
                break;
            default:
...
function handleNotification(notification, handlers) {
...
function resolveSandboxChildModule() {
    ...
    const built = fileURLToPath(new URL("../../dist/workflows/workflow-sandbox-child.js", import.meta.url));
    if (existsSync(built))
        return built;
    throw new Error(`Workflow sandbox child module is unavailable: ${adjacent}. Build the package before running workflows.`);
}
...
function sendIfConnected(child, message) {
    if (!child.connected)
        return;
    try {
        child.send(message, () => { });
...
function isWorkflowErrorCode(value) {
    return value !== undefined && Object.values(WorkflowErrorCode).includes(value);
}
