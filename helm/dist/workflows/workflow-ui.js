...
function agentStall(a, now) {
    if (a.status !== "running" || typeof a.lastEventAt !== "number")
        return "";
...
function rightAgentRow(a, selected, modelColStart, innerW, theme, now) {
...
function renderSinglePane(state, phases, selPhaseIdx, agents, width, theme, bodyCap, inAgents, now) {
...
export function renderNavigator(state, model, width, theme = PLAIN, viewportRows = 24, now = Date.now()) {
    const lines = [];
    ...
    if (state.kind === "runs") {
        ...
        if (a) {
            ...
            const stall = agentStall(a, now);
            if (stall)
                body.push(theme.fg("warning", `⚠ ${stall}`));
            if (a.model)
                body.push(dim("Model: ") + (shortModel(a.model) ?? ""));
            if (a.error)
                body.push(dim("Error: ") + a.error);
            if (a.errorCode)
                body.push(`${dim("Error code: ")}${a.errorCode}${a.recoverable ? " (recoverable)" : ""}`);
        ...
        if (w) {
            const body = [];
            if (w.description)
                body.push(dim("Description: ") + w.description);
...
function historyLabel(entry) {
    if (entry.kind === "toolCall")
        return entry.toolName ? `assistant tool ${entry.toolName}` : "assistant tool";
    if (entry.role === "tool")
        return entry.toolName ? `tool ${entry.toolName}` : "tool";
    if (entry.kind === "error")
        return `${entry.role} error`;
    return entry.role;
}
...
export function keyToAction(keyId, kind, itemKind) {
    switch (keyId) {
        ...
        case "return":
        case "right":
            if (kind === "detail" || kind === "savedDetail")
                return { type: "none" };
            return { type: "drill" };
        case "escape":
        case "esc":
            ...
            switch (action.type) {
                ...
                case "save": {
                    ...
                    if (!run?.script) {
                        ...
                        registerSavedWorkflow(pi, opts.cwd ?? process.cwd(), saved, undefined, () => storage.list().some((w) => w.name === saved.name), { commandPrefix: "wf-" });
