...
export async function readUsageCache(path) {
    try {
        ...
        const snapshots = root.snapshots.map(parseSnapshot).filter((value) => value !== undefined);
        const checkedRecord = asRecord(root.checkedAt);
        const checkedAt = {};
        for (const provider of PROVIDER_ORDER) {
            const value = finiteNumber(checkedRecord?.[provider]);
            if (value !== undefined)
                checkedAt[provider] = value;
        }
        ...
        if (root.version === 1) {
            for (const snapshot of snapshots)
                checkedAt[snapshot.provider] = snapshot.fetchedAt;
        }
        return { snapshots, checkedAt };
    }
    catch {
...
export async function readSnapshotCache(path) {
    return (await readUsageCache(path)).snapshots;
}
export async function writeSnapshotCache(path, snapshots, checkedAt = {}) {
    const safe = snapshots.map((snapshot) => ({ ...snapshot, source: "extension-cache" }));
...
function parseSnapshot(value) {
    ...
    if (!record || !PROVIDER_ORDER.includes(record.provider) || !Array.isArray(record.buckets))
        return undefined;
    const fetchedAt = finiteNumber(record.fetchedAt);
    if (fetchedAt === undefined)
        return undefined;
    const buckets = record.buckets.map(parseBucket).filter((bucket) => bucket !== undefined);
    if (buckets.length === 0)
        return undefined;
    return {
