...
import { asRecord, finiteNumber, HttpError, parseTimestamp } from "../http.js";
...
export async function fetchCodexUsage(options) {
    ...
    for (const credential of credentials) {
        if (tokenExpired(credential.access, now()))
            continue;
        try {
            ...
            if (!response.ok)
                throw new HttpError(response.status, `Codex usage returned HTTP ${response.status}`);
...
export function parseCodexUsagePayload(payload, fetchedAt = Date.now()) {
...
export function parseCodexAppServerPayload(payload, fetchedAt = Date.now()) {
    const root = asRecord(payload);
    if (!root)
...
function addRateLimitBuckets(target, value, label, id, affectsHealth) {
...
function addWindow(target, value, label, id, affectsHealth) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const processLine = (line) => {
            ...
            if (message?.id === 0 && message.result) {
                ...
                if (error)
                    finish(new Error(stringValue(error.message) ?? "Codex app-server rate-limit request failed"));
                else {
                    try {
        ...
        child.stdout.on("data", (chunk) => {
            ...
            if (stdout.length > 1024 * 1024)
                return finish(new Error("Codex app-server output exceeded 1MB"));
        ...
        child.on("exit", (code) => {
            if (!settled)
                finish(new Error(stderr.trim().slice(0, 500) || `Codex app-server exited with code ${code ?? "unknown"}`));
        });
...
function tokenExpired(token, now) {
    try {
