...
export function parseClaudeUsagePayload(payload, source = "live", fetchedAt = Date.now()) {
    const root = asRecord(payload);
    if (!root)
        throw new Error("Claude usage response was not an object");
    const buckets = [];
    const known = [
    ...
    if (Array.isArray(root.limits)) {
        root.limits.forEach((item, index) => {
            ...
            if (record.kind === "session" || record.kind === "weekly_all")
                return;
            addClaudeBucket(buckets, { utilization: record.percent, resets_at: record.resets_at }, id, displayName ? `7-day · ${displayName}` : humanize(record.kind), record.is_active !== false);
        });
    }
    if (buckets.length === 0)
        throw new Error("Claude usage response contained no rate-limit windows");
    return { provider: "claude", source, fetchedAt, buckets: dedupeBuckets(buckets) };
}
async function readClaudeProviderCache(paths, now) {
    try {
...
async function readClaudeCredential(paths) {
    try {
        ...
        if (typeof oauth?.accessToken !== "string" || expiresAt === undefined)
            return undefined;
