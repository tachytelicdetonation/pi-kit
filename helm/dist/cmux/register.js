...
import { AuditLog, ClaudeFleetManager, ClaudeHookStore, CmuxClient, CmuxEventTail, loadConfig, resolveRuntimePaths, StateStore, } from "./index.js";
const permissionModes = ["plan", "manual", "acceptEdits", "dontAsk"];
const actions = ["run", "parallel", "status", "stop"];
...
export function registerClaudeCmux(pi, opts) {
    const config = loadConfig();
    const paths = resolveRuntimePaths();
    const cmux = new CmuxClient();
    const createManager = (instanceId) => {
    ...
    const makeDecider = (ctx) => async (request) => {
        ...
        if (!ctx.hasUI)
            return { action: "deny" };
        if (request.kind === "exit-plan") {
            const choice = await ctx.ui.select("Claude completed its plan", ["Continue with manual approvals", "Continue in auto mode", "Deny"], { timeout: config.permissionTimeoutMs });
            if (choice === "Continue with manual approvals")
                return { action: "plan-manual" };
            if (choice === "Continue in auto mode")
                return { action: "plan-auto" };
            return { action: "deny" };
        }
        if (request.kind === "permission") {
    ...
    const makeTrustDecider = (ctx) => async (run) => {
        const ds = opts.presence.isOpen() ? opts.presence.dataSource() : undefined;
        if (ds) {
            const idx = await bridgePermissionRequest(ds, {
        ...
        if (!ctx.hasUI)
            return false;
        return ctx.ui.confirm("Trust Claude working directory?", `${run.cwd}\n\nClaude Code will gain normal agent access to this directory.`, { timeout: config.registrationTimeoutMs });
    };
        ...
        async execute(_toolCallId, params, signal, onUpdate, ctx) {
            ...
            if (params.action === "run") {
                if (!params.prompt?.trim())
                    throw new Error("action=run requires prompt");
            ...
            if (!params.tasks?.length)
                throw new Error("action=parallel requires tasks");
    ...
    pi.registerCommand("claude-cmux", {
        ...
        handler: async (args, ctx) => {
            ...
            updateStatus(ctx, "Claude launching…");
            try {
                const result = await manager.runTask({ prompt: args, cwd: ctx.cwd }, { decider: makeDecider(ctx), trustDecider: makeTrustDecider(ctx) });
...
function formatRuns(runs) {
    if (runs.length === 0)
        return "No managed Claude sessions.";
    return runs
        .map((run) => `${run.runId}  ${run.state.padEnd(18)} ${run.name}\n` +
        `  cwd=${run.cwd}\n` +
