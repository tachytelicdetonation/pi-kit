...
test("createWorkflowTool invalid args throws descriptive error", () => {
  ...
  if (tool.prepareArguments) {
    ...
    assert.throws(() => prepare({ script: 123 }), /at least one.*scriptPath.*script.*name/);
...
test("workflowToolSchema exposes all latest source forms as optional schema fields", () => {
  ...
  assert.ok(schema.properties.script, "script should be a schema property");
  assert.ok(schema.properties.name, "name should be a schema property");
  for (const field of ["scriptPath", "script", "name", "resumeFromRunId"]) {
    assert.ok(!(schema.required ?? []).includes(field), `${field} is optional at schema level`);
  }
});
