...
export function registerEffortCommand(pi: ExtensionAPI, state: EffortState): void {
  pi.registerCommand("effort", {
    description: "Standing workflow effort: off | high | ultracode — ultracode combines xhigh reasoning with workflows",
    async handler(args: string, _ctx: ExtensionCommandContext) {
      ...
      if (arg === "off" || arg === "high" || arg === "ultra" || arg === "ultracode") {
        const level: EffortLevel = arg === "ultracode" ? "ultra" : arg;
        state.level = level;
        if (level === "high") pi.setThinkingLevel?.("high");
        if (level === "ultra") pi.setThinkingLevel?.("xhigh");
        await say(
          level === "off"
            ? "Effort off — messages are no longer auto-armed as workflows."
            : `Effort ${arg} — substantive messages now auto-arm a workflow (${level === "ultra" ? "exhaustive" : "thorough"} fan-out). Use /effort off to stop.`,
        );
        return;
      }
