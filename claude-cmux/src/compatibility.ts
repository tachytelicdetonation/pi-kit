...
export async function checkCompatibility(
  cmux: CmuxClient,
  options: { strict: boolean; claudeBin?: string; env?: NodeJS.ProcessEnv } = { strict: true },
): Promise<CompatibilityReport> {
  const warnings: string[] = [];
  const errors: string[] = [];
  let cmuxVersion: string | undefined;
  ...
  try {
    ...
    if (capabilities.protocol !== "cmux-socket") errors.push(`Unexpected cmux protocol: ${capabilities.protocol ?? "missing"}`);
    if (capabilities.access_mode !== "cmuxOnly") warnings.push(`Unexpected cmux access mode: ${capabilities.access_mode ?? "missing"}`);
    for (const method of REQUIRED_CMUX_METHODS) {
      if (!capabilities.methods.includes(method)) errors.push(`cmux is missing required method: ${method}`);
    }
  ...
  try {
    ...
    if (versionResult.code !== 0) throw new Error(versionResult.stderr || `exit ${versionResult.code}`);
    ...
    if (helpResult.code !== 0) throw new Error(helpResult.stderr || `exit ${helpResult.code}`);
    const help = `${helpResult.stdout}\n${helpResult.stderr}`;
    if (!help.includes("--permission-mode")) errors.push("Claude Code lacks --permission-mode");
    if (!/--permission-mode[\s\S]{0,600}[\"']plan[\"']/i.test(help)) {
      errors.push("Claude Code does not advertise plan permission mode");
    }
    if (!help.includes("--resume")) errors.push("Claude Code lacks --resume");
  } catch (error) {
...
function inRange(
  ...
  exclusiveMaximum: [number, number, number],
): boolean {
